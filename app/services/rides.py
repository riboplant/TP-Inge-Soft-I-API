...
def get_ride(city_from, city_to, date, people,small_packages,  medium_packages, large_packages, db):
    ...
    now = pytz.timezone('America/Argentina/Buenos_Aires')
    now_aware = timezone('America/Argentina/Buenos_Aires').localize(now)
    now_time = now_aware.time().replace(microsecond=0)
    print(now_time)
