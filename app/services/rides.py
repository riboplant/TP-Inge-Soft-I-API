...
def get_ride(city_from, city_to, date, people,small_packages,  medium_packages, large_packages, db):
    ...
    rides = db.query(Rides).filter(
        ...
        or_(
            and_(Rides.ride_date == now.date(), Rides.start_maximum_time.replace(tzinfo=None) > now_time),
            Rides.ride_date > now.date()  
        )
    ).all() 

    for ride in rides:
        print(ride.start_maximum_time.replace(tzinfo=None))
        print(now_time)
        print(ride.start_maximum_time.replace(tzinfo=None) > now_time)
        print(True)
